...
def main(cfg, gpu, save_dir):
    ...
    # for i,file in enumerate(trainloader.dataset):
    #     figure = plt.figure(figsize=(5, 5))
    #     img,label = file
    #     label = torch.tensor(np.moveaxis(label.numpy(),0,-1))
    
    #     # print(img.shape)
    #     plt.imshow(label, interpolation='nearest',vmin=0, vmax=1,aspect='auto')
    #     plt.show()
    #     # break
    ...
    for epoch in range(epochs):
        ...
        for iter, (img, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg['AMP']):
                ...
                loss = loss_fn(logits, lbl.argmax(dim=1).to(torch.int64))
